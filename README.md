...
```powershell
python -m venv venv                              # ① 建虚拟环境（只在第一次）
.\venv\Scripts\python.exe -m pip install -r requirements.txt

.\venv\Scripts\python.exe init_db.py             # ② 一键建库（幂等，可重复运行）
...
```bash
python3 -m venv venv                             # ① 建虚拟环境（只在第一次）
./venv/bin/python -m pip install -r requirements.txt

./venv/bin/python init_db.py                     # ② 一键建库
./venv/bin/python app.py                         # ③ 启动
```

浏览器打开 **<http://127.0.0.1:5001>**，演示账号 **`admin` / `123456`**，进「漏洞实验」挑一关开始。

> **只监听 127.0.0.1**：只有本机能访问，同事/同学连不上——这是故意的，安全。
> 不需要数据库服务、不需要 Docker、不需要联网。数据库文件由 `init_db.py` 从零生成。
